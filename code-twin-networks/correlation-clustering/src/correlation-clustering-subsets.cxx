...
template<class T>
void convertHDF5(
        std::filesystem::path const & hdfPath,
        std::filesystem::path const & outPathProblem,
        std::filesystem::path const & outPathFileNames,
        T const epsilon = 1e-6
        ){
    size_t numberOfElements;
    std::vector<T> affinityData;
    ...
    out.open(outPathProblem, std::ios::out);

    out << numberOfElements << std::endl;
    ...
    for (size_t i = 0; i < numberOfElements; ++i){
        for (size_t j = i + 1; j < numberOfElements; ++j){
            ...
            T cost = edgeCosts[edge.second];
            out << std::round(1000*cost);
    ...
    outFileNames.open(outPathFileNames, std::ios::out);

    for (size_t i = 0; i < fileIds.size(); ++i){
        outFileNames << i << " " << fileIds[i] << std::endl;
    }
